...
def venn3(labels, names=['A', 'B', 'C'], **options):
    ...
    edges = options.get('edges', [default_colors[i] for i in range(3)])
    ...
    # body
    draw_ellipse(fig, ax, 0.383, 0.633, 0.5, 0.5, 0.0, colors[0])
    draw_ellipse(fig, ax, 0.616, 0.633, 0.5, 0.5, 0.0, colors[1])
    draw_ellipse(fig, ax, 0.500, 0.410, 0.5, 0.5, 0.0, colors[2])
    draw_text(fig, ax, 0.50, 0.27, labels.get('001', ''), fontsize=fontsize) # 下
    draw_text(fig, ax, 0.73, 0.65, labels.get('010', ''), fontsize=fontsize) # 右
    draw_text(fig, ax, 0.64, 0.48, labels.get('011', ''), fontsize=fontsize) # 右下交
    draw_text(fig, ax, 0.27, 0.65, labels.get('100', ''), fontsize=fontsize) # 左
    draw_text(fig, ax, 0.36, 0.48, labels.get('101', ''), fontsize=fontsize) # 左下交
    draw_text(fig, ax, 0.50, 0.72, labels.get('110', ''), fontsize=fontsize) # 上交
    draw_text(fig, ax, 0.50, 0.54, labels.get('111', ''), fontsize=fontsize) # 中

    # legend
    draw_text(fig, ax, 0.20, 0.85, names[0], colors[0], fontsize=fontsize, ha="right", va="bottom")
    draw_text(fig, ax, 0.80, 0.85, names[1], colors[1], fontsize=fontsize, ha="left", va="bottom")
    draw_text(fig, ax, 0.51, 0.12, names[2], colors[2], fontsize=fontsize, va="top")
    leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
    leg.get_frame().set_alpha(0.5)
